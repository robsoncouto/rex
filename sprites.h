#ifndef SPRITES_H
#define SPRITES_H

#define CACTS1_GLCD_HEIGHT 16
#define CACTS1_GLCD_WIDTH  8
const static unsigned char __attribute__ ((progmem)) cacts1[]={
0xe0, 0xe0, 0x00, 0xff, 0xff, 0x00, 0xf8, 0xf8,
0x03, 0x07, 0x04, 0xff, 0xff, 0x01, 0x01, 0x00,
};
#define CACTS2_GLCD_HEIGHT 16
#define CACTS2_GLCD_WIDTH  8
const static unsigned char __attribute__ ((progmem)) cacts2[]={
0xf0, 0x00, 0xfe, 0xff, 0xfe, 0x00, 0xfc, 0x00,
0x07, 0x04, 0xff, 0xff, 0xff, 0x01, 0x01, 0x00,
};
#define CACTS3_GLCD_HEIGHT 16
#define CACTS3_GLCD_WIDTH  8
const static unsigned char __attribute__ ((progmem)) cacts3[]={
0x78, 0xfc, 0xc0, 0xff, 0xff, 0x00, 0xf8, 0xf8,
0x00, 0x00, 0x00, 0xff, 0xff, 0x02, 0x03, 0x01,
};

#define CACTS4_GLCD_HEIGHT 16
#define CACTS4_GLCD_WIDTH  8
const static unsigned char __attribute__ ((progmem)) cacts4[]={
0xf0, 0x00, 0xfe, 0xff, 0xfe, 0x00, 0xfc, 0x00,
0x07, 0x04, 0xff, 0xff, 0xff, 0x01, 0x01, 0x00,
};
#define CACTS5_GLCD_HEIGHT 16
#define CACTS5_GLCD_WIDTH  8
const static unsigned char __attribute__ ((progmem)) cacts5[]={
0xfc, 0xfc, 0x00, 0xff, 0xff, 0x00, 0xf0, 0xf0,
0x03, 0x07, 0x0c, 0xff, 0xff, 0x18, 0x1f, 0x0f,
};
#define CACTS6_GLCD_HEIGHT 16
#define CACTS6_GLCD_WIDTH  8
const static unsigned char __attribute__ ((progmem)) cacts6[]={
0xfc, 0x00, 0xfe, 0xff, 0xfe, 0x00, 0xfc, 0x00,
0x01, 0x01, 0xff, 0xff, 0xff, 0x02, 0x03, 0x00,
};
#define CACTUSB1_GLCD_HEIGHT 24
#define CACTUSB1_GLCD_WIDTH  12
const static unsigned char __attribute__ ((progmem)) cactusb1[]={
0x80, 0xc0, 0x80, 0x00, 0xfe, 0xff, 0xff, 0xfe, 0x00, 0xc0, 0xe0, 0xc0,
0x3f, 0x7f, 0xff, 0xe0, 0xff, 0xff, 0xff, 0xff, 0x60, 0x7f, 0x3f, 0x1f,
0x00, 0x00, 0x00, 0x00, 0x7f, 0x7f, 0x7f, 0x7f, 0x00, 0x40, 0x00, 0x00,
};
#define CACTUSB2_GLCD_HEIGHT 24
#define CACTUSB2_GLCD_WIDTH  11
const static unsigned char __attribute__ ((progmem)) cactusb2[]={
0xc0, 0xc0, 0x00, 0x00, 0xff, 0xff, 0xff, 0x00, 0x00, 0xe0, 0xc0,
0x7f, 0xff, 0xe0, 0xe0, 0xff, 0xff, 0xff, 0x60, 0x60, 0x3f, 0x1f,
0x00, 0x00, 0x00, 0x00, 0x7f, 0x7f, 0x7f, 0x00, 0x00, 0x00, 0x00,
};
#define CACTUSB3_GLCD_HEIGHT 24
#define CACTUSB3_GLCD_WIDTH  12
const static unsigned char __attribute__ ((progmem)) cactusb3[]={
0xf8, 0xf8, 0xf8, 0x00, 0xfe, 0xff, 0xff, 0xfe, 0x00, 0xc0, 0xe0, 0xc0,
0x07, 0x0f, 0x1f, 0x1c, 0xff, 0xff, 0xff, 0xff, 0x60, 0x7f, 0x3f, 0x1f,
0x00, 0x00, 0x00, 0x00, 0x7f, 0x7f, 0x7f, 0x7f, 0x00, 0x40, 0x00, 0x00,
};
#define CACTUSB4_GLCD_HEIGHT 24
#define CACTUSB4_GLCD_WIDTH  12
const static unsigned char __attribute__ ((progmem)) cactusb4[]={
0xc0, 0xc0, 0x00, 0x00, 0xff, 0xff, 0xff, 0x00, 0x00, 0xc0, 0xc0, 0x00,
0x7f, 0xff, 0xc0, 0xc0, 0xff, 0xff, 0xff, 0x40, 0x40, 0x7f, 0x3f, 0x00,
0x00, 0x00, 0x00, 0x80, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0x80,
};
#define CACTUSB5_GLCD_HEIGHT 24
#define CACTUSB5_GLCD_WIDTH  12
const static unsigned char __attribute__ ((progmem)) cactusb5[]={
0x00, 0x00, 0x00, 0xfe, 0xfe, 0x00, 0x00, 0xf0, 0x00, 0x00, 0x00, 0x00,
0xff, 0xfe, 0x80, 0xff, 0xff, 0x02, 0xe2, 0x03, 0xf8, 0xf8, 0x00, 0xf0,
0x00, 0x01, 0x01, 0xff, 0xff, 0x00, 0x07, 0x04, 0xff, 0xff, 0x02, 0x03,
};
#define CACTUSB6_GLCD_HEIGHT 24
#define CACTUSB6_GLCD_WIDTH  12
const static unsigned char __attribute__ ((progmem)) cactusb6[]={
0x00, 0xf8, 0xf8, 0x00, 0x00, 0xff, 0xff, 0xff, 0x00, 0x00, 0xc0, 0xc0,
0x00, 0x0f, 0x1f, 0x1c, 0x1c, 0xff, 0xff, 0xff, 0x40, 0x40, 0x7f, 0x3f,
0x00, 0x00, 0x00, 0x00, 0x80, 0xff, 0xff, 0xff, 0x80, 0x00, 0x00, 0x00,
};
#define DINO1_GLCD_HEIGHT 24
#define DINO1_GLCD_WIDTH  20
const static unsigned char __attribute__ ((progmem)) dino1[]={
0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0xfe, 0xff, 0xfd, 0xff, 0x7f, 0x5f,
0x5f, 0x5f, 0x1f, 0x1e,
0x1f, 0x3e, 0x7c, 0xf8, 0xf8, 0xfc, 0xfe, 0xfe, 0xff, 0xff, 0xff, 0xff, 0x7f, 0x3f, 0x02, 0x06,
0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00, 0x01, 0x1f, 0x17, 0x03, 0x01, 0x03, 0x1f, 0x10, 0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00,
};
#define DINO2_GLCD_HEIGHT 24
#define DINO2_GLCD_WIDTH  20
const static unsigned char __attribute__ ((progmem)) dino2[]={
0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0xfe, 0xff, 0xff, 0xff, 0x7f, 0x5f,
0x5f, 0x5f, 0x1f, 0x1e,
0x1f, 0x3e, 0x7c, 0xf8, 0xf8, 0xfc, 0xfe, 0xfe, 0xff, 0xff, 0xff, 0xff, 0x7f, 0x3f, 0x02, 0x06,
0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00, 0x01, 0x1f, 0x17, 0x03, 0x01, 0x03, 0x1f, 0x10, 0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00,
};
#define DINO3_GLCD_HEIGHT 24
#define DINO3_GLCD_WIDTH  20
const static unsigned char __attribute__ ((progmem)) dino3[]={
0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0xfe, 0xff, 0xfd, 0xff, 0x7f, 0x5f,
0x5f, 0x5f, 0x1f, 0x1e,
0x1f, 0x3e, 0x7c, 0xf8, 0xf8, 0xfc, 0xfe, 0xfe, 0xff, 0xff, 0xff, 0xff, 0x7f, 0x3f, 0x02, 0x06,
0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00, 0x01, 0x1f, 0x17, 0x03, 0x01, 0x01, 0x03, 0x02, 0x02, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00,
};
#define DINO4_GLCD_HEIGHT 24
#define DINO4_GLCD_WIDTH  20
const static unsigned char __attribute__ ((progmem)) dino4[]={
0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0xfe, 0xff, 0xfd, 0xff, 0x7f, 0x5f,
0x5f, 0x5f, 0x1f, 0x1e,
0x1f, 0x3e, 0x7c, 0xf8, 0xf8, 0xfc, 0xfe, 0xfe, 0xff, 0xff, 0xff, 0xff, 0x7f, 0x3f, 0x02, 0x06,
0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00, 0x01, 0x03, 0x07, 0x05, 0x01, 0x03, 0x1f, 0x10, 0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00,
};
#define DINO5_GLCD_HEIGHT 24
#define DINO5_GLCD_WIDTH  20
const static unsigned char __attribute__ ((progmem)) dino5[]={
0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0xfe, 0xff, 0xf9, 0xf9, 0x7f, 0x7f,
0x7f, 0x7f, 0x3f, 0x3e,
0x1f, 0x3e, 0x7c, 0xf8, 0xf8, 0xfc, 0xfe, 0xfe, 0xff, 0xff, 0xff, 0xff, 0x7f, 0x3f, 0x02, 0x06,
0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00, 0x01, 0x1f, 0x17, 0x03, 0x01, 0x03, 0x1f, 0x10, 0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00,
};
#define GND_GLCD_HEIGHT 8
#define GND_GLCD_WIDTH  512
const static unsigned char __attribute__ ((progmem)) gnd[]={
,
};


#endif
